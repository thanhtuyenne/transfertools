...
function Customize({
  ...
  const tts_hugging = async (text) => {
    ...
    return await fetch(`${apiUrl}/api/tts/hugging`, requestOptions)
      .then((response) => {
        return response.json();
      })
      .then((result) => {
  ...
  const sq2sq_summary_eng = async (text) => {
    ...
    return await fetch(`${apiUrl}/api/openai/eng`, requestOptions)
      .then((response) => {
        return response.json();
      })
      .then((result) => {
  ...
  const handleInitNewElement = async (newEl) => {
    ...
    try {
      switch (boxSelected.type) {
        case "Text":
          if (indexTool == 0) {
            newEl.children.ref.current.setInput(
              await tts_google(boxSelected.value)
            ); // fetch result type:data, file
          } else if (indexTool == 1) {
            newEl.children.ref.current.setInput(
              await tts_zalo(boxSelected.value, { speaker_id: 1, speed: 1 })
            );
          } else if (indexTool == 2) {
            newEl.children.ref.current.setInput(
              await tts_hugging(boxSelected.value)
            );
          } else if (indexTool == 3) {
            newEl.children.ref.current.setInput(
              await text2Image(boxSelected.value)
            );
          } else if (indexTool == 4) {
            newEl.children.ref.current.setInput(
              await sq2sq_summary_vn(boxSelected.value)
            );
          } else if (indexTool == 5) {
            newEl.children.ref.current.setInput(
              await sq2sq_summary_eng(boxSelected.value)
            );
          } else if (indexTool == 6) {
            newEl.children.ref.current.setInput(
              await sq2sq_QA(boxSelected.value)
            );
          } else {
